      ...
      <section className="pb-20 px-8">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-xl text-foreground/70 mb-10 leading-relaxed max-w-lg">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link 
                  to="/login"
                  className="bg-primary text-white px-10 py-4 rounded-full text-lg font-bold hover:bg-primary-hover transition-all shadow-xl flex items-center justify-center gap-2 w-full"
                >
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link 
                  to="/como-funciona"
                  className="bg-card text-primary border border-primary/20 px-10 py-4 rounded-full text-lg font-bold hover:bg-background transition-all flex items-center justify-center w-full"
                >
                  Saiba Mais
                </Link>
              </motion.div>
          ...
          >
            ...
            />
            <div className="absolute -bottom-6 -left-6 bg-card p-6 rounded-[32px] shadow-xl border border-border max-w-[240px]">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-3 h-3 bg-accent rounded-full animate-pulse"></div>
                <span className="text-xs font-bold uppercase tracking-widest text-foreground/50">Fila Ativa</span>
              </div>
              <p className="text-2xl font-serif font-bold text-foreground">98%</p>
              <p className="text-xs text-foreground/50">Taxa de satisfação dos pacientes no último mês.</p>
            </div>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, index) => (
              <motion.div
                ...
                className="p-8 rounded-[32px] border border-border hover:border-primary/20 hover:shadow-xl transition-all group"
              >
      ...
      <section className="py-24 px-8 bg-background">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-left">
            {[
              { title: "Redução de Espera", description: "Diminuímos em média 40% o tempo de permanência na sala de espera.", icon: CheckCircle2, color: "text-primary" },
              { title: "Dados em Tempo Real", description: "Médicos e gestores têm acesso a métricas precisas de atendimento.", icon: CheckCircle2, color: "text-secondary" },
              { title: "Fácil Integração", description: "Interface amigável que requer treinamento mínimo para a equipe.", icon: CheckCircle2, color: "text-accent" },
              { title: "Foco no Paciente", description: "Toda a jornada é pensada para reduzir o estresse do atendimento.", icon: CheckCircle2, color: "text-info" }
            ].map((item, idx) => (
              <motion.div 
                key={idx}
